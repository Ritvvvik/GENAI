...
def format_item(item):
    message = {
         ...
         "user": f"\nHere's some context: {item['context']}",
         "assistant": item["response"]
    }
    templated_message = chat_template(message)
    return {"message": templated_message}

def tokenize_function(example):
    context_length = 1024
    encoded = tokenizer(example["message"],  return_tensors="pt",
                padding="max_length",
                truncation=True,
...
if __name__ == '__main__':
    ...
    dataset = (load_dataset("databricks/databricks-dolly-15k",trust_remote_code=True, split="train")
                .shuffle(seed=1)
                .select(range(200)))
    ...
    tokenized_ds = chat_template_ds.map(tokenize_function, remove_columns='message', num_proc=os.cpu_count())
